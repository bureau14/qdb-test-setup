...
do
    ...
    ARGS_INSECURE="${COMMON_NODE_ARGS} -a ${THIS_URI_INSECURE} -r ${THIS_DATA_DIR_INSECURE} -l ${THIS_LOG_DIR_INSECURE} --enable-performance-profiling --with-firehose \$qdb.firehose"
    ...
    if [ ${QDB_ENABLE_SECURE_CLUSTER} -ne 0 ] ; then
        echo "Cluster secure:"
        ARGS_SECURE="${COMMON_NODE_ARGS} -a ${THIS_URI_SECURE} -r ${THIS_DATA_DIR_SECURE} -l ${THIS_LOG_DIR_SECURE} --enable-performance-profiling  --with-firehose \$qdb.firehose --security=true --cluster-private-file=${CLUSTER_PRIVATE_KEY} --user-list=${USER_LIST}"
